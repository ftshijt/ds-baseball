...
real = (np.array(real)).reshape(num_test,1)

accuracy_class = np.sum(classification_pred == real)/num_test*100
accuracy_clus_kmeans = (np.sum(clustering_pred_kmeans == real))/num_test*100
accuracy_clus_kmedians = (np.sum(clustering_pred_kmedians == real))/num_test*100
accuracy_clus_gmm = (np.sum(clustering_pred_gmm == real))/num_test*100
accuracy_clus_spectral = (np.sum(clustering_pred_spectral == real))/num_test*100
